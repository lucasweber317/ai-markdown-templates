---
name: roadmap-comms
description: Turn roadmap content into audience-specific communications — executive one-pagers, sales enablement, or customer-facing previews.
---
...
Work conversationally. The user specifies which roadmap slice, which audience(s), and roughly which format.
...
If they have a roadmap doc, use it. Otherwise ask them to paste the relevant section.
...
- Reference the strategic themes from the user's strategic themes
...
- Use appropriate terminology from the user's glossary (if provided)
...
1. **Use strategic themes from the user's roadmap**
   - Connect features to established themes
   - Show coherent strategy, not random features

2. **Use product terminology from the user's glossary (if provided)**
